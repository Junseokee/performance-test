...
resource "aws_iam_policy" "ebs_csi_iam_policy" {
  name        = "dps-test-AmazonEKS_EBS_CSI_Driver_Policy"
  path        = "/"
...
resource "aws_iam_role" "ebs_csi_iam_role" {
  name = "dps-test-ebs-csi-iam-role"
  ...
  tags = {
    tag-key = "dps-test-ebs-csi-iam-role"
  }
}
...
resource "aws_eks_addon" "app_cluster_ebs_csi" {
  depends_on                  = [aws_iam_role_policy_attachment.ebs_csi_iam_role_policy_attach, aws_eks_node_group.cd_nodes]
